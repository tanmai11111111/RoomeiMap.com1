...
class UserLocationManager extends MapManager {
  ...
  this.firebaseManager.db.ref('manualLocations').once('value', (snapshot) => {
    ...
    if (sortType === 'distance' && sortOrder === 'asc') {
      ...
      locations.sort((a, b) => {
        const priceA = parseFloat(a.price) || Infinity; // Đặt giá trị lớn nếu price không hợp lệ
        const priceB = parseFloat(b.price) || Infinity;
        return priceA - priceB;
      });
    } else if (sortType === 'price' && sortOrder === 'desc') {
      locations.sort((a, b) => {
        const priceA = parseFloat(a.price) || -Infinity; // Đặt giá trị nhỏ nếu price không hợp lệ
        const priceB = parseFloat(b.price) || -Infinity;
